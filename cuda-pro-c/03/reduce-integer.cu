...
__global__ void reduceUnrolling4(int *g_idata, int *g_odata, unsigned int n)
{
    ...
    // unrolling 4 (4 blocks as one group)
    if (idx + blockDim.x < n)
    {
        // independent memory load/store ops
        int a1 = g_idata[idx];
...
int main(int argc, char const *argv[])
{
    ...
    if (gpu_sum != cpu_sum)
        std::cout << "Test result failed: sum result not match!" << std::endl;
    ...
    std::cout << "GPU reduce unrolling 8: " << gpu_duration << " nanosec gpu_sum: "
              << gpu_sum << " with " << improved << "% improved!" << std::endl;
    if (gpu_sum != cpu_sum)
        std::cout << "Test result failed: sum result not match!" << std::endl;
